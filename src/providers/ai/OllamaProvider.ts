...
export class OllamaProvider implements IAIProvider {
  ...
  async initialize(): Promise<void> {
    if (!this.initializationPromise) {
      this.initializationPromise = this.initializeOllama();
    }
    await this.initializationPromise;
  }

  protected async initializeOllama(): Promise<void> {
    if (this.config.type !== 'ollama') {
      throw new Error('Invalid provider type for OllamaProvider');
    }
    try {
      // Default endpoint for Ollama
      const endpoint = this.config.endpoint || 'http://localhost:11434/v1';
      
      logger.debug(`Initializing Ollama client with endpoint: ${endpoint}`);

      // Initialize OpenAI client with Ollama endpoint
      this.openAIClient = new OpenAI({
        apiKey: this.config.apiKey || 'ollama', // Ollama doesn't require API key, but OpenAI client requires it
        baseURL: endpoint,
      });
  ...
  async generateQuery(request: QueryGenerationRequest): Promise<NLQuery> {
    ...
    try {
      logger.debug('Generating KQL query with Ollama...');
      ...
      const response = await this.openAIClient.chat.completions.create({
        model: this.config.model || 'phi3:latest',
        messages: [
  ...
  async regenerateQuery(request: RegenerationRequest): Promise<NLQuery> {
    ...
    try {
      logger.debug(`Regenerating KQL query (attempt ${request.context.attemptNumber}) with Ollama...`);
  ...
  async analyzeQueryResult(request: QueryAnalysisRequest): Promise<QueryAnalysisResult> {
    ...
    try {
      logger.debug(`Starting ${request.analysisType} analysis with Ollama`);
  ...
  protected extractReasoningFromResponse(content: string): string {
    ...
    try {
      const jsonContent = this.extractJSONFromResponse(content);
      const parsed = JSON.parse(jsonContent);
      return parsed.reasoning || 'Generated by Ollama (local LLM)';
    } catch {
      // Try direct JSON parsing (for responses without markdown)
      try {
        const parsed = JSON.parse(content);
        ...
        return 'Generated by Ollama (local LLM, non-JSON response)';
      }
    }
  }
