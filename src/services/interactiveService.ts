...
import { ConsoleOutputRenderer } from './ConsoleOutputRenderer';
...
export class InteractiveService {
  ...
  constructor(
    ...
    this.outputRenderer = new ConsoleOutputRenderer({
  ...
  private async handleInteractiveOutput(result: QueryResult, executionTime: number, originalQuery?: string): Promise<void> {
    ...
    // Use the new OutputRenderer for display
    await this.outputRenderer.displayResult(result, executionTime, originalQuery);

    // Show chart for numeric data
    await this.outputRenderer.showChart(result);
    ...
    await this.outputRenderer.handleFileOutput(result);
  }
  ...
  private async promptForAnalysisOption(result: QueryResult, originalQuery: string): Promise<void> {
    ...
    try {
      ...
      this.outputRenderer.displayAnalysis(analysis, analysisType);
